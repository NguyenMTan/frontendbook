...
const DetailProduct: React.FC = () => {
  ...
  const mutateCart = useMutation({
    ...
    onError: () => {
      toast.remove("1");
      toast.error("Số lượng không được dưới 0!");
    },
  });

  const handleAddProduct = () => {
    if (quantity > product.quantity) {
      toast.error(`Sản phẩm chúng tôi chỉ còn ${product.quantity}`);
      return;
    }
    if (userInfo) {
        ...
        <div className="flex h-96 w-1/3 items-center justify-center ">
          ...
          <Carousel className="w-full max-w-xs">
            <CarouselContent>
              ...
              {product.productImages.map((i: images) => (
      ...
      <div className="container mt-2 h-60  border-t-2">
        <div className="mx-20 flex">
          ...
          {content === "fullDescription" && (
            <div className="ml-4 p-4">{product.fullDescription}</div>
          )}
          {content === "details" && (
            <div className="ml-4 w-72 p-4">
              {product.productDetails.map((d: details) => (
